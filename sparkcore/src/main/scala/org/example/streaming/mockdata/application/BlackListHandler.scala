...
import org.example.streaming.mockdata.dbutils.JdbcUtil
...
object BlackListHandler {
  ...
  def addBlackList(filterAdsLogDSteam: DStream[Ads_log]): Unit = {
    ...
    val dateUserAdToOne: DStream[((String, String, String), Long)] = filterAdsLogDSteam.map(
      adsLog => {
      //a.将时间戳转换为日期字符串
    ...
    dateUserAdToCount.foreachRDD(rdd => {
      rdd.foreachPartition(iter => {
        ...
        iter.foreach {
          case ((dt, user, ad), count) => JdbcUtil.executeUpdate(
            connection,
            """
